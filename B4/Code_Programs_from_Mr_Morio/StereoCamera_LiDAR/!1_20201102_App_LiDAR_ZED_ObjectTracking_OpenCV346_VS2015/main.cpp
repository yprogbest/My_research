...
int main()
{
	...
	Eigen::Vector3d eigen_T_LiDAR2ZED;
	Eigen::MatrixXd eigen_R_LiDAR2ZED(3, 3);
	...
	for (int i = n_From; i <= n_To; i++) {
		...
		while (fscanf_s(fp_lidar_point_cloud, "%lf\t%lf\t%lf\t%lf", &point.x, &point.y, &point.z, &point.intensity) != EOF) {
			...
			if (flag_nan == true) {
				...
				// change the coordinates of xyz -> yzx
...
*/
		...
		for (size_t i = 0; i < point_cloud_LiDAR_yzx.size(); i++) {
			...
			point_cloud_LiDAR2ZED.push_back(buf_LiDAR2ZED_xyz);
