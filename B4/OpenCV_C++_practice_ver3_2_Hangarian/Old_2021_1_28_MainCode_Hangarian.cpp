...
int yolo_lidar() {
			...
			if (fgets(yolo_bb_file_BUFFER, YOLO_FILE_LINE_MAX_BUFFER_SIZE, fp_yolo_bb) != NULL)
			{
				...
				else
				{
					...
					else
					{
						...
						//2021_1_28 add
						//縮小した場合に使う
						cv::rectangle(img_out, cv::Point(int(yolo_buf_bbox.x + (1.0 / 5.0)*yolo_buf_bbox.width), int(yolo_buf_bbox.y + (1.0 / 5.0)*yolo_buf_bbox.height)), cv::Point(int(yolo_buf_bbox.x + (4.0 / 5.0)*yolo_buf_bbox.width), int(yolo_buf_bbox.y + (4.0 / 5.0)*yolo_buf_bbox.height)), cv::Scalar(0, 0, 0), 1);
					...
					for (int j = 0; j < (int)imagePoints_LiDAR2ZED.size(); j++)
					{
						...
						//if ((imagePoints_LiDAR2ZED[j].x >= yolo_buf_bbox.x && imagePoints_LiDAR2ZED[j].x <= yolo_buf_bbox.x + yolo_buf_bbox.width + (1.0 / 5.0)*yolo_buf_bbox.width) && (imagePoints_LiDAR2ZED[j].y >= yolo_buf_bbox.y - (1.0 / 5.0)*yolo_buf_bbox.height && imagePoints_LiDAR2ZED[j].y <= yolo_buf_bbox.y + yolo_buf_bbox.height))
	...
	ofs_BBox_container.open("D:\\MyTemp\\BBox_container.txt");
	...
	ofs_BBox_person.open("D:\\MyTemp\\BBox_person.txt");
