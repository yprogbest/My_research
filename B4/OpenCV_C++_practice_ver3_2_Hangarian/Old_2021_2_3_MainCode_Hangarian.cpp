			...
			if (fgets(yolo_bb_file_BUFFER, YOLO_FILE_LINE_MAX_BUFFER_SIZE, fp_yolo_bb) != NULL)
			{
				...
				else
				{
					...
					if (NN.size() != 0)
					{
						...
						{
							fprintf_s(fp_w_NN, "%d\t%lf\t%lf\t%lf\t%d\n", i_yolo_lidar, all_NN_representative[i].cx, all_NN_representative[i].cy, all_NN_representative[i].cz, all_NN_representative[i].count);
						}
...
int NN_method_representative(vector<struct NN_type> nn, vector<struct NN_type> &all_nn_representative, double object_range, int Thresh_clustering_num)
{
	...
	min_distance = sqrt((nn[0].cx * nn[0].cx) + (nn[0].cy * nn[0].cy) + (nn[0].cz * nn[0].cz));
