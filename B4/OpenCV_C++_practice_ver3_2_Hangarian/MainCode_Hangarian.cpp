...
double masuda_calc_IoU(vector<vector<double>> &all_d_IoU_container, vector<vector<double>> &all_d_IoU_person);
			...
			if (fgets(yolo_bb_file_BUFFER, YOLO_FILE_LINE_MAX_BUFFER_SIZE, fp_yolo_bb) != NULL)
			{
				...
				else
				{
					...
					if (all_NN_representative.size() != 0)
					{
						...
						if (transporting)
						{
							//cv::putText(img_out, min_sitance_num_new + "m", cv::Point(int(yolo_buf_bbox.x - 40), int(yolo_buf_bbox.y)), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 0, 0), 1);
							continue;
						}
						else
						{
...
double masuda_calc_IoU(vector<vector<double>> &all_d_IoU_container, vector<vector<double>> &all_d_IoU_person)
{
	...
	vector<double> buf_d_IoU_container;


	double d_IoU_person;
	vector<double> buf_d_IoU_person;
	...
	for (size_t num = 0; num < IN_yolo_BBox_buf_container.size(); num++)
	{
		...
		for (size_t f = 0; f < In_yolo_BBox_container.size(); f++)
		{
			...
			{
				buf_d_IoU_container.clear();    buf_d_IoU_container.shrink_to_fit();
				...
				for (size_t k = 0; k < Objects_yolo_BBox_container.size(); k++)
				{
					...
					if (d_IoU_container > IoU_max_container)
					{
						...
						buf_d_IoU_container.push_back(IoU_max_container);

						if (all_d_IoU_container.size() <= 1)
						{
							all_d_IoU_container.push_back(buf_d_IoU_container);
						}
					}
				...
				if (IoU_max_container > IoU_thresh)
				{
					...
					if (all_d_IoU_container.size() >= 2)
					{
						all_d_IoU_container[n_object_IoU_max_id_container].push_back(IoU_max_container);
					}
				}
				else
				{
	...
	for (size_t num = 0; num < IN_yolo_BBox_buf_person.size(); num++)
	{
		...
		for (size_t f = 0; f < In_yolo_BBox_person.size(); f++)
		{
			...
			{
				buf_d_IoU_person.clear();    buf_d_IoU_person.shrink_to_fit();
				...
				for (size_t k = 0; k < Objects_yolo_BBox_person.size(); k++)
				{
					...
					if (d_IoU_person > IoU_max_person)
					{
						...
						buf_d_IoU_person.push_back(IoU_max_person);

						if (all_d_IoU_person.size() <= 1)
						{
							all_d_IoU_person.push_back(buf_d_IoU_person);
						}
					}
				...
				if (IoU_max_person > IoU_thresh)
				{
					...
					if (all_d_IoU_person.size() >= 2)
					{
						all_d_IoU_person[n_object_IoU_max_id_person].push_back(IoU_max_person);
					}
				}
				else
				{
...
//2021_2_3 add for calculating the representative clustering
int NN_method_representative(vector<struct NN_type> nn, vector<struct NN_type> &all_nn_representative, double object_range, int Thresh_clustering_num)
{
	...
	/*for (size_t i = 0; i < all_nn_representative.size(); i++)
	{
		printf("all_nn_representative = %lf\t%lf\t%lf\t%d\n", all_nn_representative[i].cx, all_nn_representative[i].cy, all_nn_representative[i].cz, all_nn_representative[i].count);
...
//2021_2_3 add for drawing the moving trajectory in Gnuplot
int moving_trajectory_gnuplot(vector<vector<struct yolo_bounding_box>> input_yolo_data, vector<Point2f> input_lidar_2d, vector<Point3f> input_lidar_3d)
{
	FILE *gid;
	gid = _popen("C:/Win64App/gnuplot/bin/gnuplot.exe", "w");
