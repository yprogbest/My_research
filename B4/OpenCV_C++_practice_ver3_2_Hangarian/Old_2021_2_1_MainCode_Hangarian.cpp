...
int yolo_lidar();
int get_mediam(std::string outputfile_path);
int median(vector<double> input_data_distance_3D, vector<Point3f> input_data_xyz_3D);
...
int calculate_Representative_LiDAR_point(vector<Point2f> all_point_cloud_in_a_boundingbox_2d, std::vector<Point3f>all_point_cloud_in_a_boundingbox_3d,
	vector<Point2f> &all_distance_from_min_container_2d, vector<Point2f> &all_distance_from_min_person_2d,
	vector<double> &all_distance_from_min_container_3d, vector<double> &all_distance_from_min_person_3d);





cv::Point3f in_obj_points_container;
std::vector<cv::Point3f> d_median_points_container;
cv::Point3f in_obj_points_person;
std::vector<cv::Point3f> d_median_points_person;
...
std::vector<Point3f>all_point_cloud_in_a_boundingbox_3D_container_for_gnuplot;
cv::Point3f point_cloud_in_a_boundingbox_3D_container_for_gnuplot;

std::vector<Point3f>all_point_cloud_in_a_boundingbox_3D_person_for_gnuplot;
cv::Point3f point_cloud_in_a_boundingbox_3D_person_for_gnuplot;
...
vector<Point2f>all_distance_from_min_container_2D;
vector<Point2f>all_distance_from_min_person_2D;

vector<double> all_distance_from_min_container_3D;
vector<double> all_distance_from_min_person_3D;
...
float WidthFromPointcloud = 1.0;
double HeightFromPintcloud = (double)WidthFromPointcloud;


//define the person's point cloud that the most number of container's point cloud included.   
float PersonPointCloud_in_Boundingbox_x;
double PersonPointCloud_in_Boundingbox_distance;
		...
		while (1)
		{
			...
			if (fgets(yolo_bb_file_BUFFER, YOLO_FILE_LINE_MAX_BUFFER_SIZE, fp_yolo_bb) != NULL)
			{
				...
				else
				{
					...
					for (size_t a = 0; a < (int)all_distance_from_min_container_2D.size(); a++)
					{
						...
						//LiDARの点群を画像に出力
						cv::circle(img_out, cv::Point((int)all_distance_from_min_container_2D[a].x, (int)all_distance_from_min_container_2D[a].y), 3, cv::Scalar(0, 255, 0), 1, cv::LINE_8);
					...
					//2021_2_1 add
					all_distance_from_min_container_2D.clear();                all_distance_from_min_container_2D.shrink_to_fit();
					all_distance_from_min_person_2D.clear();                   all_distance_from_min_person_2D.shrink_to_fit();

					all_distance_from_min_container_3D.clear();                all_distance_from_min_container_3D.shrink_to_fit();
					all_distance_from_min_person_3D.clear();                   all_distance_from_min_person_3D.shrink_to_fit();
...
int get_mediam(std::string outputfile_path)
{
	...
	for (int a = 0; a < (int)all_lidar_distance_in_boundingbox.size(); a++)
	{
		for (int b = a + 1; b < (int)all_lidar_distance_in_boundingbox.size(); b++)
		{
			if (all_lidar_distance_in_boundingbox[a] < all_lidar_distance_in_boundingbox[b])
			{
				...
				//3D
				tmp_x = all_point_cloud_in_a_boundingbox_3D[b].x;
				all_point_cloud_in_a_boundingbox_3D[b].x = all_point_cloud_in_a_boundingbox_3D[a].x;
				all_point_cloud_in_a_boundingbox_3D[a].x = tmp_x;

				tmp_y = all_point_cloud_in_a_boundingbox_3D[b].y;
				all_point_cloud_in_a_boundingbox_3D[b].y = all_point_cloud_in_a_boundingbox_3D[a].y;
				all_point_cloud_in_a_boundingbox_3D[a].y = tmp_y;

				tmp_z = all_point_cloud_in_a_boundingbox_3D[b].z;
				all_point_cloud_in_a_boundingbox_3D[b].z = all_point_cloud_in_a_boundingbox_3D[a].z;
				all_point_cloud_in_a_boundingbox_3D[a].z = tmp_z;



				//2D
				tmp_x_2D = all_point_cloud_in_a_boundingbox_2D[b].x;
				all_point_cloud_in_a_boundingbox_2D[b].x = all_point_cloud_in_a_boundingbox_2D[a].x;
				all_point_cloud_in_a_boundingbox_2D[a].x = tmp_x_2D;

				tmp_y_2D = all_point_cloud_in_a_boundingbox_2D[b].y;
				all_point_cloud_in_a_boundingbox_2D[b].y = all_point_cloud_in_a_boundingbox_2D[a].y;
				all_point_cloud_in_a_boundingbox_2D[a].y = tmp_y_2D;
	...
	{
		//C++
		outputfile << a << "\t" << all_lidar_distance_in_boundingbox[a] << "\t" << all_point_cloud_in_a_boundingbox_3D[a].x << "\t" << all_point_cloud_in_a_boundingbox_3D[a].y << "\t" << all_point_cloud_in_a_boundingbox_3D[a].z << "\n";
	}
	...
	//2021_1_31 add
	calculate_Representative_LiDAR_point(all_point_cloud_in_a_boundingbox_2D, all_point_cloud_in_a_boundingbox_3D, all_distance_from_min_container_2D, all_distance_from_min_person_2D, all_distance_from_min_container_3D, all_distance_from_min_person_3D);
	...
	//median(all_distance_from_min_container_3D, all_point_cloud_in_a_boundingbox_3D);
	...
	vector<double> &all_distance_from_min_container_3d, vector<double> &all_distance_from_min_person_3d)
{
	...
	std::vector<Point2f> all_point_cloud_in_a_boundingbox_2d_container;
	cv::Point2f point_cloud_in_a_boundingbox_2d_container;
	
	std::vector<Point2f> all_point_cloud_in_a_boundingbox_2d_person;
	cv::Point2f point_cloud_in_a_boundingbox_2d_person;

	std::vector<Point3f>all_point_cloud_in_a_boundingbox_3d_container;
	cv::Point3f point_cloud_in_a_boundingbox_3d_container;

	std::vector<Point3f>all_point_cloud_in_a_boundingbox_3d_person;
	cv::Point3f point_cloud_in_a_boundingbox_3d_person;
	...
	double distance_person_3d;

	vector<double> Pointcloud_in_BB_pointcloud_distance_container;
	vector<double> Pointcloud_in_BB_pointcloud_distance_person;


	double min_distance_container_3d;
	double min_distance_person_3d;

	double distance_from_min_container_3d;
	double distance_from_min_person_3d;

	vector<Point2f> Pointcloud_in_BB_pointcloud_container_2d;
	vector<Point2f> Pointcloud_in_BB_pointcloud_person_2d;

	vector<Point3f> Pointcloud_in_BB_pointcloud_container_3d;
	vector<Point3f> Pointcloud_in_BB_pointcloud_person_3d;
	...
	for (size_t i = 0; i < all_point_cloud_in_a_boundingbox_2d.size(); i++)
	{
		...
		{
			//2D
			point_cloud_in_a_boundingbox_2d_container.x = all_point_cloud_in_a_boundingbox_2d[i].x;
			point_cloud_in_a_boundingbox_2d_container.y = all_point_cloud_in_a_boundingbox_2d[i].y;

			all_point_cloud_in_a_boundingbox_2d_container.push_back(point_cloud_in_a_boundingbox_2d_container);



			//3D
			point_cloud_in_a_boundingbox_3d_container.x = all_point_cloud_in_a_boundingbox_3d[i].x;
			point_cloud_in_a_boundingbox_3d_container.y = all_point_cloud_in_a_boundingbox_3d[i].y;
			point_cloud_in_a_boundingbox_3d_container.z = all_point_cloud_in_a_boundingbox_3d[i].z;

			all_point_cloud_in_a_boundingbox_3d_container.push_back(point_cloud_in_a_boundingbox_3d_container);

		}
		else if (person)
		{
			//2D
			point_cloud_in_a_boundingbox_2d_person.x = all_point_cloud_in_a_boundingbox_2d[i].x;
			point_cloud_in_a_boundingbox_2d_person.y = all_point_cloud_in_a_boundingbox_2d[i].y;

			all_point_cloud_in_a_boundingbox_2d_person.push_back(point_cloud_in_a_boundingbox_2d_person);



			//3D
			point_cloud_in_a_boundingbox_3d_person.x = all_point_cloud_in_a_boundingbox_3d[i].x;
			point_cloud_in_a_boundingbox_3d_person.y = all_point_cloud_in_a_boundingbox_3d[i].y;
			point_cloud_in_a_boundingbox_3d_person.z = all_point_cloud_in_a_boundingbox_3d[i].z;

			all_point_cloud_in_a_boundingbox_3d_person.push_back(point_cloud_in_a_boundingbox_3d_person);
		}
	...
	//container
	for (size_t i = 0; i < all_point_cloud_in_a_boundingbox_2d_container.size(); i++)
	{
		...
		//printf("container\n\n");
		//printf("all_point_cloud_in_a_boundingbox_2d_container[%d]=%f\t%f\n\n\n", i, all_point_cloud_in_a_boundingbox_2d_container[i].x, all_point_cloud_in_a_boundingbox_2d_container[i].y);




		for (size_t j = 0; j < all_point_cloud_in_a_boundingbox_2d_container.size(); j++)
		{
			if (j != i)
			{

				if ((all_point_cloud_in_a_boundingbox_2d_container[j].x >= all_point_cloud_in_a_boundingbox_2d_container[i].x - threshold_2d) &&
					(all_point_cloud_in_a_boundingbox_2d_container[j].x <= all_point_cloud_in_a_boundingbox_2d_container[i].x + threshold_2d) &&
					(all_point_cloud_in_a_boundingbox_2d_container[j].y >= all_point_cloud_in_a_boundingbox_2d_container[i].y - threshold_2d) &&
					(all_point_cloud_in_a_boundingbox_2d_container[j].y <= all_point_cloud_in_a_boundingbox_2d_container[i].y + threshold_2d))
				{
					//コマンドプロンプト上に書き出す
					//書き出す項目↓
					//obuject name, all_point_cloud_in_a_boundingbox_2d_container[i], all_point_cloud_in_a_boundingbox_2d_container[j]
					


					//std::cout << all_point_cloud_in_a_boundingbox_2d_container[j] << std::endl;
					//std::cout << all_point_cloud_in_a_boundingbox_3d_container[j] << std::endl;



					Pointcloud_in_BB_pointcloud_container_2d.push_back(all_point_cloud_in_a_boundingbox_2d_container[j]);    //2D push_back
					Pointcloud_in_BB_pointcloud_container_3d.push_back(all_point_cloud_in_a_boundingbox_3d_container[j]);    //3D push_back


					distance_container_3d = sqrt((all_point_cloud_in_a_boundingbox_3d_container[j].x*all_point_cloud_in_a_boundingbox_3d_container[j].x) + (all_point_cloud_in_a_boundingbox_3d_container[j].y*all_point_cloud_in_a_boundingbox_3d_container[j].y) + (all_point_cloud_in_a_boundingbox_3d_container[j].z*all_point_cloud_in_a_boundingbox_3d_container[j].z));
					Pointcloud_in_BB_pointcloud_distance_container.push_back(distance_container_3d);  //3D distance push_back
		...
		min_distance_container_3d = Pointcloud_in_BB_pointcloud_distance_container[0];


		for (size_t a = 0; a < Pointcloud_in_BB_pointcloud_distance_container.size(); a++)
		{

			if (Pointcloud_in_BB_pointcloud_distance_container[a] < min_distance_container_3d)
			{
				min_distance_container_3d = Pointcloud_in_BB_pointcloud_distance_container[a];
			}
		...
		//printf("min_distance_container_3d = %lf\n\n", min_distance_container_3d);
		...
		for (size_t a = 0; a < Pointcloud_in_BB_pointcloud_distance_container.size(); a++)
		{
			...
			distance_from_min_container_3d = Pointcloud_in_BB_pointcloud_distance_container[a] - min_distance_container_3d;


			if (distance_from_min_container_3d <= threshold_distance_3d)
			{
				
				//printf("Pointcloud_in_BB_pointcloud_container_2d[%d] = %lf\t%lf\n", a, Pointcloud_in_BB_pointcloud_container_2d[a].x, Pointcloud_in_BB_pointcloud_container_2d[a].y);
				//printf("Pointcloud_in_BB_pointcloud_distance_container[%d] = %lf\n\n", a, Pointcloud_in_BB_pointcloud_distance_container[a]);


				//2D
				all_distance_from_min_container_2d.push_back(Pointcloud_in_BB_pointcloud_container_2d[a]);


				//3D distance
				all_distance_from_min_container_3d.push_back(Pointcloud_in_BB_pointcloud_distance_container[a]);
	...
	/*for (size_t a = 0; a < all_distance_from_min_container_2D.size(); a++)
	{
		printf("all_distance_from_min_container_2D[%d] = %f\t%f\n", a, all_distance_from_min_container_2D[a].x, all_distance_from_min_container_2D[a].y);
	}*/
	...
	//clear
	all_point_cloud_in_a_boundingbox_2d_container.clear();             all_point_cloud_in_a_boundingbox_2d_container.shrink_to_fit();
	all_point_cloud_in_a_boundingbox_2d_person.clear();                all_point_cloud_in_a_boundingbox_2d_person.shrink_to_fit();
	all_point_cloud_in_a_boundingbox_3d_container.clear();             all_point_cloud_in_a_boundingbox_3d_container.shrink_to_fit();
	all_point_cloud_in_a_boundingbox_3d_person.clear();                all_point_cloud_in_a_boundingbox_3d_person.shrink_to_fit();
	Pointcloud_in_BB_pointcloud_container_2d.clear();                  Pointcloud_in_BB_pointcloud_container_2d.shrink_to_fit();
	Pointcloud_in_BB_pointcloud_container_3d.clear();                  Pointcloud_in_BB_pointcloud_container_3d.shrink_to_fit();
	Pointcloud_in_BB_pointcloud_distance_container.clear();            Pointcloud_in_BB_pointcloud_distance_container.shrink_to_fit();
...
int gnuplot_sort_result()
{
	...
	struct sort_data  median_result;
