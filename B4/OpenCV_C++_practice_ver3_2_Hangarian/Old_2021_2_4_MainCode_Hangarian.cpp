...
int NN_method_representative(vector<struct NN_type> nn, vector<struct NN_type> &all_nn_representative, double object_range, int Thresh_clustering_num)
{
	...
	for (size_t i = 0; i < nn.size(); i++)
	{
		if (nn[i].count >= Thresh_clustering_num)
		{
			...
			if (distance[i] < (min_distance + object_range))
			{
...
//2021_2_3 add for drawing to Gnuplot the result of the distance which was calculated by NN method.
int NN_distance_gnuplot(vector<struct NN_type> &all_nn_representative)
{
	FILE *gid;
	gid = _popen("C:/Win64App/gnuplot/bin/gnuplot.exe", "w");
	...
	if (container)
	{
		fprintf(gid, "set output 'D:\\Experiments\\YOLO\\result_!YOLO_MASUDA4_20000_Hangarian_method\\NN_method_20201117_low_top_half3_distance_gnuplot\\%dcontainer_objnum.png'\n", i_yolo_lidar);
	}
	else if (person)
	{
		fprintf(gid, "set output 'D:\\Experiments\\YOLO\\result_!YOLO_MASUDA4_20000_Hangarian_method\\NN_method_20201117_low_top_half3_distance_gnuplot\\%person_objnum.png'\n", i_yolo_lidar);
	}
