...
std::vector<cv::Point3f> d_median_points;
...
double median_distance_container[NumFromMedian];

vector<double>all_median_distance_person;
double median_distance_person[NumFromMedian];


vector<Point3f>all_median_num_container;
Point3f median_num_container[NumFromMedian];


vector<Point3f>all_median_num_person;
Point3f median_num_person[NumFromMedian];
...
float PersonPointCloud_in_Boundingbox_x[NumFromMedian];
double PersonPointCloud_in_Boundingbox_distance[NumFromMedian];

vector<float>all_PersonPointCloud_in_Boundingbox_x;
vector<double>all_PersonPointCloud_in_Boundingbox_distance;


//define the value of upper left and bottom right.
float upper_left_x[NumFromMedian];
double upper_left_distance[NumFromMedian];
float bottom_right_x[NumFromMedian];
double bottom_right_diatance[NumFromMedian];

//define array
vector<float> all_upper_left_x;
vector<double>all_upper_left_distance;
vector<float> all_bottom_right_x;
vector<double> all_bottom_right_diatance;
...
int yolo_lidar() {
		...
		while (1)
		{
			...
			if (fgets(yolo_bb_file_BUFFER, YOLO_FILE_LINE_MAX_BUFFER_SIZE, fp_yolo_bb) != NULL)
			{
				...
				if (n_yolo_bb_file_stream_size == 2)
				{
					...
					//2021_1_19 addition
					//Enclose a bounding box around a person's LiDAR point cloud
					//surround_pointcloud();
					...
					//2021_1_19 addition
					all_PersonPointCloud_in_Boundingbox_x.clear();        all_PersonPointCloud_in_Boundingbox_x.shrink_to_fit();
					all_PersonPointCloud_in_Boundingbox_distance.clear(); all_PersonPointCloud_in_Boundingbox_distance.shrink_to_fit();
	...
	_pclose(gid); //close gnuplot 



	/*std::ofstream ofs;
	ofs.open("D:\\MyTemp\\all_median_points_mine.txt");
	for (size_t i = 0; i < d_median_points.size(); i++) {
		ofs << d_median_points[i].x << "\t" << d_median_points[i].y << "\t" << d_median_points[i].z << "\t" << std::endl;
	}
	ofs.close();*/
...
int median()
{
	...
	if ((int)all_lidar_distance_in_boundingbox.size() == 0)
	{
		...
		for (int b = 0; b < NumFromMedian; b++)
		{
			median_distance_container[b] = 0.0;
			median_num_container[b].x = 0.0;
			median_num_container[b].y = 0.0;
			median_num_container[b].z = 0.0;


			median_distance_person[b] = 0.0;
			median_num_person[b].x = 0.0;
			median_num_person[b].y = 0.0;
			median_num_person[b].z = 0.0;
	...
	else if ((int)all_lidar_distance_in_boundingbox.size() % 2 == 1)
	{
		...
		if (container)
		{
			...
			for (int a = (int)((int)all_lidar_distance_in_boundingbox.size() / 2 - int(NumFromMedian / 2)); a <= (int)((int)all_lidar_distance_in_boundingbox.size() / 2 + int(NumFromMedian / 2)); a++)
			{
				

				median_distance_container[array_num] = all_lidar_distance_in_boundingbox[a];
				median_num_container[array_num].x = all_point_cloud_in_a_boundingbox_3D[a].x;
				median_num_container[array_num].y = all_point_cloud_in_a_boundingbox_3D[a].y;
				median_num_container[array_num].z = all_point_cloud_in_a_boundingbox_3D[a].z;



				//2021_1_21 addition
				if (array_num == int(NumFromMedian / 2))
				{
					in_object_container.x = median_num_container[array_num].x;
					in_object_container.y = median_num_container[array_num].y;
					in_object_container.z = median_num_container[array_num].z;
		...
		else if (person)
		{
			...
			for (int a = (int)((int)all_lidar_distance_in_boundingbox.size() / 2 - int(NumFromMedian / 2)); a <= (int)((int)all_lidar_distance_in_boundingbox.size() / 2 + int(NumFromMedian / 2)); a++)
			{
				...
				median_distance_person[array_num] = all_lidar_distance_in_boundingbox[a];
				median_num_person[array_num].x = all_point_cloud_in_a_boundingbox_3D[a].x;
				median_num_person[array_num].y = all_point_cloud_in_a_boundingbox_3D[a].y;
				median_num_person[array_num].z = all_point_cloud_in_a_boundingbox_3D[a].z;


				//2021_1_21 addition
				if (array_num == int(NumFromMedian / 2))
				{
					in_object_person.x = median_num_person[array_num].x;
					in_object_person.y = median_num_person[array_num].y;
					in_object_person.z = median_num_person[array_num].z;
			...
			//median_distance_person[int(NumFromMedian / 2)] = all_lidar_distance_in_boundingbox[(int)((int)all_lidar_distance_in_boundingbox.size() / 2)];
			//median_num_person[int(NumFromMedian / 2)].x = all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2)].x;
	...
	else if((int)all_lidar_distance_in_boundingbox.size() % 2 == 0)
	{
		...
		if (container)
		{
			...
			for (int a = (int)((double)((((int)all_lidar_distance_in_boundingbox.size() / 2 - 1) + ((int)all_lidar_distance_in_boundingbox.size() / 2)) / 2.0) - (double)(int(NumFromMedian / 2) - 1)); a <= (int)((double)((((int)all_lidar_distance_in_boundingbox.size() / 2 - 1) + ((int)all_lidar_distance_in_boundingbox.size() / 2)) / 2.0) + (double)(int(NumFromMedian / 2))); a++)
			{


				//もし，中央値なら...（中央値は小数になるため，別で処理を設ける必要がある．）
				if (array_num == (int)(NumFromMedian / 2))
				{
					median_distance_container[array_num] = (all_lidar_distance_in_boundingbox[(int)((int)all_lidar_distance_in_boundingbox.size() / 2) - 1] + all_lidar_distance_in_boundingbox[(int)((int)all_lidar_distance_in_boundingbox.size() / 2)]) / 2;
					median_num_container[array_num].x = (all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2) - 1].x + all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2)].x) / 2;
					median_num_container[array_num].y = (all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2) - 1].y + all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2)].y) / 2;
					median_num_container[array_num].z = (all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2) - 1].z + all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2)].z) / 2;
		...
		else if (person)
		{
			...
			for (int a = (int)((double)((((int)all_lidar_distance_in_boundingbox.size() / 2 - 1) + ((int)all_lidar_distance_in_boundingbox.size() / 2)) / 2.0) - (double)(int(NumFromMedian / 2) - 1)); a <= (int)((double)((((int)all_lidar_distance_in_boundingbox.size() / 2 - 1) + ((int)all_lidar_distance_in_boundingbox.size() / 2)) / 2.0) + (double)(int(NumFromMedian / 2))); a++)
			{
				...
				if (array_num == (int)(NumFromMedian / 2))
				{
					median_distance_person[array_num] = (all_lidar_distance_in_boundingbox[(int)((int)all_lidar_distance_in_boundingbox.size() / 2) - 1] + all_lidar_distance_in_boundingbox[(int)((int)all_lidar_distance_in_boundingbox.size() / 2)]) / 2;
					median_num_person[array_num].x = (all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2) - 1].x + all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2)].x) / 2;
					median_num_person[array_num].y = (all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2) - 1].y + all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2)].y) / 2;
					median_num_person[array_num].z = (all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2) - 1].z + all_point_cloud_in_a_boundingbox_3D[(int)((int)all_lidar_distance_in_boundingbox.size() / 2)].z) / 2;
				...
				if (array_num == int(NumFromMedian / 2))
				{
					cv::Point3f in_obj_points;
					in_obj_points.x = in_object_container.x;
					in_obj_points.y = in_object_container.y;
					in_obj_points.z = in_object_container.z;

					d_median_points.push_back(in_obj_points);
	...
	//extract five points from the median(push_back)
	//The fifth point is median value
	for (int a = 0; a < NumFromMedian; a++)
	{
		all_median_distance_container.push_back(median_distance_container[a]);
		all_median_num_container.push_back(median_num_container[a]);

		all_median_distance_person.push_back(median_distance_person[a]);
		all_median_num_person.push_back(median_num_person[a]);
	}
...
int vector_test_sample(FILE *gid)
{
	...
	double d_object_distance_thresh = 1.5;
	...
	for (size_t num = 0; num < IN_Objects_buf_container.size(); num++)
	{
		...
		{//each frame
			...
			else
			{
				...
				{
					for (size_t l = 0; l < Objects_container[k].size(); l++)
					{
						d_dx_container = (in_object_container.x - Objects_container[k][l].x);
						d_dy_container = (in_object_container.y - Objects_container[k][l].y);
						d_dz_container = (in_object_container.z - Objects_container[k][l].z);
	...
	for (size_t num = 0; num < IN_Objects_buf_person.size(); num++)
	{
		...
		{//each frame
			...
			else
			{
				...
				{
					for (size_t l = 0; l < Objects_person[k].size(); l++)
					{
						d_dx_person = (in_object_person.x - Objects_person[k][l].x);
						d_dy_person = (in_object_person.y - Objects_person[k][l].y);
						d_dz_person = (in_object_person.z - Objects_person[k][l].z);
...
//2021_1_19 addition(It doesn't get well.)
//Enclose a bounding box around a person's LiDAR point cloud
...
int surround_pointcloud()
{
	...
	//count the number of the container in the bounding box of a person's LiDAR point cloud.
	int num_container_in_person_boundingbox[100];


	int a_max = 0;
	int max;

	max = num_container_in_person_boundingbox[0];
	...
	//make bounding box(person)
	for (int a = 0; a < (int)all_median_num_person.size(); a++)
	{
		
		upper_left_x[a] = all_median_num_person[a].x - WidthFromPointcloud;
		upper_left_distance[a] = all_median_distance_person[a] - HeightFromPintcloud;
		bottom_right_x[a] = all_median_num_person[a].x + WidthFromPointcloud;
		bottom_right_diatance[a] = all_median_distance_person[a] + HeightFromPintcloud;



		all_upper_left_x.push_back(upper_left_x[a]);
		all_upper_left_distance.push_back(upper_left_distance[a]);
		all_bottom_right_x.push_back(bottom_right_x[a]);
		all_bottom_right_diatance.push_back(bottom_right_diatance[a]);
	...
	//initialization
	for (int a = 0; a < (int)all_upper_left_x.size(); a++)
	{
		num_container_in_person_boundingbox[a] = 0;
	}



	//serch the person's bounding box
	for (int a = 0; a < (int)all_median_num_person.size(); a++)
	{
		//serch the container's point cloud
		for (int b = 0; b < (int)all_median_num_container.size(); b++)
		{
			//If the point cloud of container is in the bounding box of person's LiDAR point cloud,
			if ((all_median_num_container[b].x >= all_upper_left_x[a] && all_median_distance_container[b] >= all_upper_left_distance[a]) &&
				(all_median_num_container[b].x <= all_bottom_right_x[a] && all_median_distance_container[b] <= all_bottom_right_diatance[a]))
			{

				//count
				num_container_in_person_boundingbox[a]++;
	...
	//Extract the container's point cloud in the person's bounding box
	for (int a = 0; a < (int)all_upper_left_x.size(); a++)
	{

		//If there are not person's point cloud...
		if (!person)
		{
			PersonPointCloud_in_Boundingbox_x[a] = 0.0;
			PersonPointCloud_in_Boundingbox_distance[a] = 0.0;


			all_PersonPointCloud_in_Boundingbox_x.push_back(PersonPointCloud_in_Boundingbox_x[a]);
			all_PersonPointCloud_in_Boundingbox_distance.push_back(PersonPointCloud_in_Boundingbox_distance[a]);
		}
		...
		if (num_container_in_person_boundingbox[a] > max)
		{
			max = num_container_in_person_boundingbox[a];
			a_max = a;

			//If more than tree points are included
			if (num_container_in_person_boundingbox[a_max] >= 3.0)
			{
				PersonPointCloud_in_Boundingbox_x[a_max] = all_median_num_person[a_max].x;
				PersonPointCloud_in_Boundingbox_distance[a_max] = all_median_distance_person[a_max];
			...
			all_PersonPointCloud_in_Boundingbox_x.push_back(PersonPointCloud_in_Boundingbox_x[a_max]);
			all_PersonPointCloud_in_Boundingbox_distance.push_back(PersonPointCloud_in_Boundingbox_distance[a_max]);
...
int show_objects_pose_via_gnuplot(FILE *gid)
{
	...
	for (size_t i = 0; i < Objects_container.size(); i++)
	{
		for (size_t j = 0; j < Objects_container[i].size(); j++)
		{
			...
			{
				fprintf(gid, "%lf\t%lf\n", Objects_container[i][j].x, Objects_container[i][j].z);
			}			
		}
	...
	for (size_t k = 0; k < Objects_person.size(); k++)
	{
		for (size_t l = 0; l < Objects_person[k].size(); l++)
		{
			...
			{
				fprintf(gid, "%lf\t%lf\n", Objects_person[k][l].x, Objects_person[k][l].z);
			}
		}
...
int gnuplot_mapping()
{
	...
	//fprintf_s(gid, "plot '-' with points ps 1 lc rgb 'red' title 'Container in bounding box', '-' with points ps 1 lc rgb 'blue' title 'Person in bounding box', '-' with points ps 1 lc rgb 'green' title 'Transporting'\n");
	...
	//for (int a = 0; a < (int)all_PersonPointCloud_in_Boundingbox_x.size(); a++)
	//{
		//fprintf(gid, "%f\t%lf\n", all_PersonPointCloud_in_Boundingbox_x[a], all_PersonPointCloud_in_Boundingbox_distance[a]);
	//}
