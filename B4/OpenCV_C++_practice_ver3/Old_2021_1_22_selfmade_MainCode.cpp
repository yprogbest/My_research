...
vector<struct tracking_object_type> label;
vector<vector<struct tracking_object_type>> Labels;
...
int yolo_lidar() {
	...
	std::ofstream ofs;
	ofs.open("C:\\MyTemp\\all_median_points_mine.txt");
	...
	else if ((int)all_lidar_distance_in_boundingbox.size() % 2 == 1)
	{
		...
		if (container)
		{
			...
			for (int a = (int)((int)all_lidar_distance_in_boundingbox.size() / 2 - int(NumFromMedian / 2)); a <= (int)((int)all_lidar_distance_in_boundingbox.size() / 2 + int(NumFromMedian / 2)); a++)
			{
				...
				{
					in_object.x = median_num_container[array_num].x;
					in_object.y = median_num_container[array_num].y;
					in_object.z = median_num_container[array_num].z;


					//IN_Objects.push_back(in_object);
				}
		...
		else if (person)
		{
			...
			for (int a = (int)((int)all_lidar_distance_in_boundingbox.size() / 2 - int(NumFromMedian / 2)); a <= (int)((int)all_lidar_distance_in_boundingbox.size() / 2 + int(NumFromMedian / 2)); a++)
			{
				...
				if (array_num == int(NumFromMedian / 2))
				{
					in_object.x = median_num_person[array_num].x;
					in_object.y = median_num_person[array_num].y;
					in_object.z = median_num_person[array_num].z;
		...
		else if (person)
		{
			...
			for (int a = (int)((double)((((int)all_lidar_distance_in_boundingbox.size() / 2 - 1) + ((int)all_lidar_distance_in_boundingbox.size() / 2)) / 2.0) - (double)(int(NumFromMedian / 2) - 1)); a <= (int)((double)((((int)all_lidar_distance_in_boundingbox.size() / 2 - 1) + ((int)all_lidar_distance_in_boundingbox.size() / 2)) / 2.0) + (double)(int(NumFromMedian / 2))); a++)
			{
				...
				if (array_num == int(NumFromMedian / 2))
				{
					...
					in_obj_points.x = in_object.x;
					in_obj_points.y = in_object.y;
					in_obj_points.z = in_object.z;
	...
	{ //each frame
		...
		else
		{
			...
			for (size_t k = 0; k < Objects.size(); k++)
			{
				...
				{
					d_dx = (in_object.x - Objects[k][a].x);
					d_dy = (in_object.y - Objects[k][a].y);
					d_dz = (in_object.z - Objects[k][a].z);


					d_object_distance[a] = sqrt(d_dx * d_dx + d_dy * d_dy + d_dz * d_dz);
				}


				double d_object_distance_min = d_object_distance[0];


				for (int a = 0; a < n_object_tracked_count; a++)
				{
					if (d_object_distance[a] <= d_object_distance_min)
					{
						d_object_distance_min = d_object_distance[a];
