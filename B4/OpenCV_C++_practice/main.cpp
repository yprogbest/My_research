...
int yolo_lidar();
int make_histgram(std::string outputfile_path);
...
int yolo_lidar() {
	...
	//hThread_gnuplot = (HANDLE)_beginthreadex(NULL, 0, gnuplot_mapping, "gnuplot", 0, NULL); //スレッドの開始
	...
	while (1)
	{
		...
		while (1)
		{
			...
			if (fgets(yolo_bb_file_BUFFER, YOLO_FILE_LINE_MAX_BUFFER_SIZE, fp_yolo_bb) != NULL)
			{
				...
				else
				{
					...
					cv::putText(img_out, std::to_string(a_max) + "m", cv::Point(int(yolo_buf_bbox.x + yolo_buf_bbox.width + 5), int(yolo_buf_bbox.y)), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 1);
		...
		//Bounding box内のLiDAR点群をgnuplotに表示（2021_1_6追記） (2つ目のgnuplot_mapping)←たぶん使わない
		//gnuplot_mapping();
...
int make_histgram(std::string outputfile_path)
{
	...
	for (int a = 0; a < (int)all_lidar_distance_in_boundingbox.size(); a++)
	{
		...
		//{
			...
			{
				z_class = int(all_lidar_distance_in_boundingbox[a] + 1.0); //例)8.8→9
			}
			else
			{
				z_class = int(all_lidar_distance_in_boundingbox[a]);  //例)8.4 →　8
			}
...
int gnuplot_mapping()
{
	...
	fprintf(gid, "set output 'D:\\Experiments\\YOLO\\result_!YOLO_MASUDA4_20000_ver1_histgram\\mapping_image_20201117_low\\image%d.png'\n", i_yolo_lidar);
	//fprintf(gid, "set output 'D:\\Experiments\\YOLO\\result_!YOLO_MASUDA4_20000_ver1_histgram\\mapping_image_20201223112457\\image%d.png'\n", i_yolo_lidar);
